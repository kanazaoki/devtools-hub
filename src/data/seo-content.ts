...
export const seoContent: Record<string, SeoArticleData> = {
  ...
  'http-cache-builder': {
    ...
    sections: [
      { title: 'max-age と s-maxage の違い', body: 'max-ageはブラウザとCDN両方に適用されるキャッシュ有効期間（秒）です。s-maxageはCDN（共有キャッシュ）にのみ適用され、max-ageを上書きします。「ブラウザは1時間、CDNは1日キャッシュしてほしい」場合はCache-Control: max-age=3600, s-maxage=86400と設定します。CDNを使わない構成ではs-maxageは無視されるため、CDNがある場合のみ有効に機能します。CloudFrontやFastlyなどのCDNはs-maxageを優先し、それ以降のリクエストはオリジンサーバーへのアクセスなしにキャッシュから応答します。APIレスポンスのCDNキャッシュ時間とブラウザのキャッシュ時間を個別に調整したい場面で有効です。' },
      { title: 'no-cache と no-store の違い', body: 'no-cacheはキャッシュを保存するが毎回オリジンに再検証する設定です（ETagやLast-Modifiedで304が返ればキャッシュを利用）。no-storeはキャッシュ自体を一切保存せずディスクにも書き込まれません。セキュリティ要件が高い認証済みページや個人情報を含むレスポンスにはno-storeを使い、動的だが帯域節約したいページにはno-cacheを使います。no-cacheとmust-revalidateの組み合わせは再検証が失敗した場合に古いキャッシュを返さないことを保証します。プライベートキャッシュのみに保存させたい場合はprivateディレクティブを追加することでCDNなど共有キャッシュに保存されないようにできます。' },
      { title: 'stale-while-revalidate でUXを向上', body: 'stale-while-revalidateを設定すると、max-age経過後でも指定秒数内はバックグラウンドで再検証しながら古いキャッシュをすぐに返します。ユーザーには常にキャッシュから即座にレスポンスが届き、バックグラウンドで更新が走るため体感速度が大幅に改善します。例：Cache-Control: max-age=60, stale-while-revalidate=300とすると、60秒後〜360秒後の間は古いキャッシュを返しつつバックグラウンドで最新データを取得します。Next.jsのrevalidateオプションやVercelのEdge Cacheもこの仕組みを活用しており、静的生成ページを定期更新するISR（Incremental Static Regeneration）と同様の考え方です。APIのリアルタイム性と応答速度のバランスを取る場面で非常に有効です。' },
  ...
  'css-selector-tester': {
    ...
    sections: [
      { title: '基本セレクター：クラス・ID・要素', body: '.classはクラス属性でマッチ、#idはID属性でマッチ、要素名（p/div/a）はそのタグ全てにマッチします。複数のクラスを組み合わせる.class1.class2（スペースなし）はどちらのクラスも持つ要素のみにマッチします。コンマ区切り（.a, .b）でいずれかにマッチするセレクターリストを書けます。全称セレクター（*）はすべての要素にマッチしますが詳細度は0なので他のセレクターに必ず負けます。IDセレクターは詳細度が高いため使い回しにくく、現代のCSS設計ではクラスベースのセレクターが推奨されます。CSS Selector Testerでは任意のHTML文字列に対してセレクターをリアルタイムでテストし、マッチした要素を即座にハイライト表示できます。' },
      { title: '擬似クラス：:nth-child・:not・:hover', body: ':nth-child(n)は親要素内でn番目の子要素を選択します。:nth-child(2n)で偶数番目、:nth-child(odd)で奇数番目、:nth-child(3n+1)で3つおきの要素を選択できます。:not(.class)は指定クラスを持たない要素を選択し、除外フィルタに使えます。:first-child・:last-child・:only-childも頻出の擬似クラスです。:hover・:focus・:active・:visitedはユーザー操作の状態に反応する擬似クラスです。:is()はセレクターリストをまとめて書くのに便利で、:is(h1, h2, h3) { color: red }は3つの見出しに一括適用できます。これらをCSS Selector Testerで試すと直感的に動作が理解できます。' },
      { title: '結合子：子孫・子・隣接・兄弟', body: 'スペースは子孫結合子（.parent .child）で親の中の全子孫にマッチ、>は子結合子で直接の子のみマッチ、+は隣接兄弟結合子で直後の要素のみ、~は兄弟結合子で後続の全兄弟にマッチします。複雑な結合子は意図しないマッチが起きやすく、CSS Selector Testerで確認するのが確実です。たとえば「input:checked + label」はチェック状態のinputの直後にあるlabelのみにスタイルを当てられ、JavaScriptなしのカスタムチェックボックスUIの実装に使われます。「article > p:first-child」のように子結合子と擬似クラスを組み合わせることで、特定の親の直接の子である最初の段落だけを選択するような精密なスタイル指定が可能です。' },
      { title: '属性セレクターの活用', body: '[attr]はattr属性を持つ要素、[attr="val"]はattrが"val"の要素、[attr^="val"]は"val"で始まる要素、[attr$="val"]は"val"で終わる要素、[attr*="val"]は"val"を含む要素を選択します。input[type="text"]でテキスト入力のみ、a[href^="https"]でHTTPSリンクのみ、a[href$=".pdf"]でPDFリンクのみにスタイルを当てるのが代表的な使い方です。[attr~="val"]はスペース区切りの単語として"val"を含む要素を選択し、クラス属性の部分マッチに利用できます。属性セレクターはクラスを付けなくてもHTML標準属性を使って要素を特定できるためマークアップをシンプルに保てます。フォームのバリデーション状態も:validや:invalidの擬似クラスと組み合わせると効果的です。' },
    ],
  },
  ...
  'css-unit-converter': {
    ...
    sections: [
      { title: 'px（ピクセル）とremの違い', body: 'pxはデバイスピクセルを基準とした絶対単位です。ブラウザのフォントサイズ設定を変更してもpxは変わりません。remはrootのem、つまりhtmlタグのfont-sizeを基準にした相対単位です。多くのブラウザのデフォルトは16pxなので1rem=16pxですが、ユーザーがブラウザのフォントサイズを変更するとremも変わります。アクセシビリティの観点からfont-sizeにはpxよりremが推奨されます。視力が弱いユーザーはブラウザの設定でフォントサイズを大きくしていることがあり、pxで固定するとその設定が効かなくなります。WCAGガイドラインでは「テキストを200%まで拡大してもコンテンツが損なわれないこと」が求められており、remやem使用が有効な対策です。CSS Unit Converterでpxとremの換算表を即座に確認できます。' },
      { title: 'emとremの使い分け', body: 'emは現在の要素のfont-sizeを基準にした相対単位です。1.5emは「現在のフォントサイズの1.5倍」を意味します。ネストが深くなると基準が変わって計算が複雑になりますが、コンポーネント内でフォントサイズに比例した余白を設定するpadding: 0.5emのような使い方では有効です。見出しのline-heightなどにemを使うと直感的です。一方remは常にhtmlのfont-sizeが基準なのでネストの深さに影響されません。一般的な使い分けとしては、font-size・marginのようなグローバルな値にはrem、同じコンポーネント内でfont-sizeに連動してほしいpadding・line-heightにはemを使います。メディアクエリのブレークポイントにemを使うとブラウザのフォントサイズ設定に連動するアクセシブルなレイアウトが実現できます。' },
      { title: 'vw・vhの使いどころ', body: 'vwはビューポート幅の1%、vhはビューポート高さの1%を表します。100vhで全画面高さのヒーローセクション、50vwで画面の半分の幅など大きなレイアウトに使います。ただしモバイルのアドレスバーの表示/非表示でvhが変動する問題があり、svh（Small Viewport Height）・dvh（Dynamic Viewport Height）・lvh（Large Viewport Height）という新しい単位が登場しています。dvhはアドレスバーの状態に動的に追従し、svhはアドレスバー表示時の高さを基準にします。フォントサイズの流体スケーリングにはclamp()とvwを組み合わせたclamp(16px, 2.5vw, 24px)という記法が有効で、画面サイズに応じてフォントが自然にスケールします。CSS Unit Converterでvw/vhをpx換算で確認できます。' },
      { title: 'pt・cm・mmは印刷向け', body: 'pt（ポイント）・cm・mmは印刷・PDF生成向けの単位で、画面表示には基本的に使いません。1ptは1/72インチ（約0.353mm）で、Wordや印刷物のフォントサイズに使われます。Webコンテンツでは基本的にpx・rem・em・vw/vhを使い分けます。ただしCSSの@media print内での印刷スタイルではptやcmが有効です。PDFを生成するライブラリ（jsPDF・puppeteerなど）ではmm単位でページサイズを指定することがあります。CSS Unit Converterではこれらすべての単位を相互変換でき、ベースフォントサイズとビューポートサイズをカスタマイズして正確なrem/vwの換算値を確認できます。印刷デザインとWeb実装の両方を担当する場面で役立ちます。' },
    ],
  },
  ...
  'text-case-converter': {
    ...
    sections: [
      { title: '主な4種類のケース記法', body: 'camelCase（例: myVariableName）はJavaScript・Java・Swiftの変数名で標準です。PascalCase（例: MyClassName）はクラス名・ReactコンポーネントやTypeScriptの型名に使用します。snake_case（例: my_variable_name）はPython・Ruby・DBカラム名で主流で、読みやすさが高いとされています。kebab-case（例: my-component-name）はCSSクラス名・HTMLデータ属性・URLスラッグで使われます。どのケースを選ぶかはプロジェクトの言語・フレームワーク・既存コードの慣習に従うべきで、一貫性が最も重要です。Text Case Converterで単語を一度入力するだけで4種類すべてに即変換し、コピーできます。' },
      ...
      { title: 'APIフィールド名変換の実践', body: 'REST APIのJSONレスポンスはcamelCaseが多いですが、データベースのカラム名はsnake_caseが一般的です。バックエンドとフロントエンドのデータ連携やORMのマッピングでケース変換が発生します。Text Case Converterで変数名を一度入力すれば8形式すべてに変換してコピーできるため、命名規則の変換作業を効率化できます。たとえばデータベースのカラム名「user_created_at」をAPIのJSONフィールド「userCreatedAt」に変換し、フロントの型定義やZodスキーマに使う場面でツールを使うと変換ミスがなくなります。チームが複数の言語（Python/JavaScript/SQLなど）を使う環境では同じ概念のフィールド名が異なるケース記法で並存するため、変換パターンを一覧で確認できるツールは特に役立ちます。' },
  ...
  'character-counter': {
    ...
    sections: [
      { title: 'SNS・Webサービスの文字数制限', body: 'X（Twitter）の1ポスト上限は140文字（英語は280文字）、Instagram キャプションは2,200文字（ハッシュタグは30個以内）、YouTubeタイトルは100文字・説明欄は5,000文字、Meta description（SEO）は120〜160文字が推奨です。文字数カウンターでテキストを貼り付けるだけでリアルタイムにカウントされるため、制限超過を防げます。FacebookページのPostは63,206文字まで入力できますが、表示は約500文字でトランケートされます。Zennの記事タイトルは60文字以内が推奨で、それ以上は一覧画面で省略表示されます。各サービスの制限をその都度調べる手間なく、文字数カウンターで入力しながら即座に確認できます。' },
      { title: '全角・半角の文字数カウント', body: '日本語の全角文字と英数字（半角）は入力文字としては1文字ですが、テキストのバイト数は全角が3バイト（UTF-8）・半角が1バイトと異なります。データベースのVARCHAR制限やAPIのリクエストサイズ制限はバイト数で制限されていることがあるため、バイト数の確認も重要です。MySQLのVARCHAR(255)はutf8mb4（4バイト文字対応）で設定されている場合、255文字ではなく255バイトが上限になります。絵文字は1文字でUTF-8で4バイトを占有するため、絵文字を多用するテキストは文字数とバイト数が大きくかけ離れることがあります。文字数カウンターでは文字数・バイト数・全角文字数・半角文字数をそれぞれ表示してDBや入力バリデーション設計の参考にできます。' },
      { title: '読了時間の計算方法', body: '日本語の平均読書速度は1分間に400〜600文字とされています。文字数カウンターでは文字数をもとに読了時間を自動計算します。ブログ記事・技術ドキュメントの「〇分で読めます」表示は読者の閲覧判断に役立ち、直帰率改善に効果があります。英語テキストの場合は200〜250wpm（words per minute）が標準的な読書速度で、単語数をもとに計算します。Medium・Zennなどのプラットフォームが記事一覧で読了時間を表示するのは、読者が時間コストを事前に把握してクリックするかどうかを判断できるためです。長い記事には目次を付けてセクション単位で読めるように構成することが直帰率対策として有効です。文字数カウンターで文字数・単語数・読了時間を同時に確認できます。' },
      { title: '400字詰め原稿用紙換算と単語数', body: '400字詰め原稿用紙換算は書籍の分量の目安として使われます。文字数÷400で何枚分かを計算します。文庫本1冊は約250〜300枚（10万〜12万文字）が一般的です。英語テキストの場合はスペース区切りで単語数もカウントします。単語数は英語の読了時間計算（平均200〜250wpm）にも使えます。ライターや編集者が原稿料を計算する際に文字数は重要な指標で、「1文字○円」という単価設定が一般的です。また、SEOの観点では記事の文字数とGoogle検索順位の相関を指摘する研究もあります。一般に情報が充実した記事（2,000文字以上）ほど上位表示されやすい傾向があります。文字数カウンターで原稿用紙換算と合わせて確認できます。' },
    ],
  },
  ...
  'jwt-decoder': {
    ...
    sections: [
      { title: 'JWTの3つの構成要素', body: 'JWTは「ヘッダー」「ペイロード」「署名」の3パートをドット（.）でつなげた文字列です。ヘッダーはトークンタイプ（typ: JWT）と署名アルゴリズム（alg: HS256など）をBase64URLエンコードしたもの、ペイロードはクレーム（sub・iss・exp・iatなどの標準クレームとカスタムクレーム）、署名はヘッダーとペイロードをシークレットキーでHMAC計算したものです。JWT DecoderではこれらをリアルタイムにデコードしてJSON形式で表示します。HS256（HMAC-SHA256）は共有シークレット方式で、RS256（RSA-SHA256）は公開鍵/秘密鍵方式です。公開鍵を配布するJWKS（JSON Web Key Set）エンドポイントはOAuth 2.0・OpenID Connectで標準的に使われます。' },
      { title: 'expクレームと有効期限の確認', body: 'ペイロードのexp（expiration）クレームはUnixタイムスタンプで有効期限を表します。JWT Decoderでは有効期限の残り時間を人間が読める形で表示し、期限切れの場合は赤く警告します。「なぜかAPIが401を返す」という場面でトークンのexpを確認するのが基本的なデバッグ手順です。iat（issued at）クレームはトークンが発行された時刻、nbf（not before）クレームはその時刻以前には使えないという制限を表します。アクセストークンの有効期間は一般的に15分〜1時間程度に短く設定し、リフレッシュトークンで長期認証を管理するパターンがセキュリティの観点から推奨されます。JWT Decoderでexpとiatの差分からトークンの寿命を計算して設計の確認に使えます。' },
      { title: 'JWTはなぜBase64なのか', body: 'JWTのヘッダーとペイロードはBase64URL（URL-safe Base64）でエンコードされているだけで、暗号化はされていません。つまりJWTは「偽造できない」が「中身は見える」という特性があります。機密情報（パスワード・クレジットカード番号・個人を特定できる情報）はペイロードに含めてはいけません。改ざん検知には署名（第3パート）を使います。暗号化が必要な場合はJWE（JSON Web Encryption）という別の規格を使います。JWT Decoderのようなツールで誰でもデコードできることを念頭に置き、ペイロードには最小限のクレームだけ含める設計が安全です。userIdのような識別子はOKですが、メールアドレスや氏名などの個人情報はペイロードに含めないことを推奨します。' },
      { title: 'JWTのデバッグとよくある問題', body: '「署名が無効」「トークンが期限切れ」「issuerが一致しない」はJWT関連のよくあるエラーです。JWT Decoderでトークンをデコードしてiss（issuer）・aud（audience）・exp・iatを確認することで問題を特定できます。オフラインで動作するためトークンを外部サービスに送信するリスクがありません。よくある問題のもう一つは「アルゴリズムの混同」で、サーバーがHS256を期待しているのにRS256で署名されたトークンを渡すとエラーになります。ヘッダーのalgクレームで使用アルゴリズムを確認してください。また「audクレームの不一致」は複数サービス（APIサーバーAとBなど）でJWTを共用する際に発生しやすく、audで用途を絞ることでセキュリティを高めつつデバッグも容易になります。' },
    ],
  },
  ...
  'hash-studio': {
    ...
    sections: [
      { title: 'MD5・SHA-1はなぜ非推奨なのか', body: 'MD5はハッシュ衝突（異なるデータが同じハッシュになる）が2004年に発見されており、デジタル署名・証明書・セキュリティ用途には使えません。SHA-1も2017年にGoogleが「SHAttered攻撃」でPDFの衝突を実証し、現在はブラウザ・認証局ともに廃止済みです。ただしファイルチェックサムの確認など、攻撃者がデータを操作できない場面では速度の面から引き続き使われています。Git内部のハッシュもSHA-1でしたが（現在SHA-256移行中）、これはバージョン追跡の用途なのでセキュリティ攻撃の文脈とは異なります。新規プロジェクトではSHA-256以上を使うのが原則です。' },
      { title: 'SHA-256とSHA-512の違い', body: 'SHA-256は256ビット（64文字の16進数）のハッシュを生成し、現在最も広く使われているアルゴリズムです。SHA-512は512ビット（128文字の16進数）で、より衝突耐性が高く64ビット演算が最適化されているため64ビットCPUでは高速です。TLS証明書・Git・ブロックチェーン・JWTの署名（HS256/RS256）など多くのシステムがSHA-256を採用しています。SHA-3（Keccak）はSHA-2と並列して設計された新世代のアルゴリズムで、SHA-2に脆弱性が発見されたときの代替として策定されましたが、現時点でSHA-2に致命的な問題はなく移行は限定的です。ブラウザのWebCrypto APIはSHA-256・SHA-384・SHA-512をサポートしており、JavaScriptからハッシュ計算が可能です。' },
      { title: 'ファイルの整合性検証', body: 'ソフトウェアのダウンロードページではSHA-256のチェックサムが公開されることが多く、ダウンロードしたファイルが改ざんされていないか確認するために使います。Hash Studioではファイルをドロップするだけでハッシュ値を生成し、既知のハッシュ値との一致を確認（Verify）できます。Linuxディストリビューション・Pythonインタープリタ・OpenSSLなどのオープンソースソフトウェアは必ずSHA-256チェックサムを公開しており、ダウンロード後に必ず検証することがセキュリティの基本です。CI/CDパイプラインでも依存パッケージのハッシュを固定する「ロックファイル」（package-lock.json・poetry.lock）はこの整合性検証の仕組みを活用しています。Hash Studioは大容量ファイルもブラウザ内でストリーミング処理するため、サーバーに送信せず安全に検証できます。' },
      { title: 'パスワードのハッシュ化について', body: 'パスワードを平文でDBに保存するのは危険であるため、ハッシュ化して保存します。ただし単純なSHA-256ハッシュではレインボーテーブル攻撃やブルートフォース攻撃に弱いため、bcrypt・Argon2・scryptなどのパスワード専用ハッシュ関数とソルト（ランダムな付加値）を使うべきです。これらはあえて計算コストが高くなるよう設計されており、攻撃者が大量のパスワードを試す速度を下げる効果があります。Node.jsではargon2・bcryptjsパッケージが一般的で、PHPにはpassword_hash()関数が標準で搭載されています。Hash StudioはMD5・SHA-256などの汎用ハッシュ関数のツールであり、パスワードハッシュにはbcrypt等の専用ライブラリを使ってください。' },
    ],
  },
  ...
  'url-encoder': {
    ...
    sections: [
      { title: 'パーセントエンコーディングとは', body: 'URLに使えない文字（日本語・スペース・&など）は%に続く16進数2桁で表現します。「東京」はUTF-8バイト列をパーセントエンコードした%E6%9D%B1%E4%BA%ACになります。スペースは%20またはHTMLフォームデータではapplication/x-www-form-urlencodedとして+で表現されます。URLの各パートによってエンコードすべき文字が異なります。パスセグメント（/で区切られた部分）では/自体をエンコードしてはいけませんが、クエリパラメータの値には/も%2Fとしてエンコードすべきです。RFC 3986に定義される予約文字（!・\'・(・)・*）はコンテキストによってエンコードの要否が変わるため、JavaScriptのencodeURIComponentを使うのが最も安全です。' },
      ...
      { title: 'クエリパラメータの自動パース', body: 'URLにクエリパラメータが含まれる場合（例: https://example.com?q=検索語&lang=ja&sort=price%3Adesc）、URL Encoderは自動的にkeyとvalueに分解して表示します。複数パラメータの内容を一覧確認したり、エンコードされた値をデコードして読みやすくする用途に便利です。解析されたパラメータはそれぞれ個別に編集・追加・削除ができ、変更後のURLを再生成してコピーできます。ブラウザのDevToolsでコピーしたURLのクエリパラメータを素早く確認する、APIドキュメントのサンプルURLのパラメータを解読する、UTMパラメータ付きのマーケティングURLを分析するといった場面で日常的に役立ちます。URLSearchParams APIと同様のパース動作をUIで直感的に行えます。' },
      { title: 'APIデバッグでの活用', body: 'REST APIへのリクエストでクエリパラメータに日本語や記号を含む場合、エンコード漏れで404やパースエラーが発生することがあります。URL Encoderにクエリ文字列を貼り付けて正しくエンコードされているか確認し、デバッグを効率化できます。たとえばaxiosやfetchでURLを構築する際に手動連結すると特殊文字のエンコードが漏れやすく、URLSearchParamsを使った方が安全ですが、実際のリクエストURLがどうなるかを確認するのにURL Encoderが役立ちます。cURLコマンドでAPIを叩く際も日本語パラメータを含む場合はシェルでのエスケープとURLエンコードの両方が必要で、ツールで事前に正規化されたURLを確認してからコマンドに使う運用が効率的です。' },
    ],
  },
  ...
  'markdown-preview': {
    ...
    sections: [
      ...
      { title: '目次（TOC）の自動生成', body: '長いMarkdownドキュメントでは見出しから目次を自動生成して各セクションへジャンプできると便利です。Markdown Previewでは見出しH1〜H3から目次を自動生成し、クリックで対応箇所にスクロールします。技術文書・仕様書・READMEを書きながら構成を把握するのに役立ちます。GitHubのREADMEでも[[TOC]]を手動で書く代わりに、見出しのアンカーリンクを使った目次を先頭に置く慣習があります。Markdown Previewでプレビューを見ながら目次とセクションのバランスを確認することで、ドキュメントの階層構造を設計段階で整理できます。長い技術仕様書は読者が興味のある部分だけ読めるよう目次ナビゲーションが不可欠です。' },
      { title: 'HTMLエクスポートの活用（デスクトップ版）', body: 'デスクトップ版Markdown PreviewではMarkdownをHTMLファイルとしてエクスポートできます。スタイル付きのHTMLを書き出せるため、メールで送付する技術ドキュメントや社内向けの静的HTMLページを手軽に作成できます。.mdファイルのドラッグ&ドロップ読み込みにも対応しており、既存のMarkdownファイルをそのまま開いてプレビューできます。エクスポートされたHTMLはCSSをインラインに含むself-contained形式のため、外部リソースなしにどの環境でも同じ見た目で表示されます。GitHubのREADMEに似たスタイルで書き出せるため、CMS・メール・Slack共有など多様な用途に対応できます。PandocなどのCLIツールが使えない環境でも手軽に変換できるのが利点です。' },
    ],
  },
  ...
  'uuid-generator': {
    ...
    sections: [
      { title: 'UUID v4（ランダム）', body: 'UUID v4は暗号論的に安全な乱数から生成されるため、推測が困難です。最も広く使われるバージョンで、ほとんどのユースケースに適しています。「xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx」の形式で、バージョン番号（4）とバリアントビット（8・9・a・bのいずれか）を除いて122ビットがランダムです。衝突確率は極めて低く、1兆件のUUIDを生成しても衝突が起きる確率は10億分の1以下です。JavaScriptではcrypto.randomUUID()（Node.js 14.17+・最新ブラウザ）でライブラリなしにUUID v4を生成できます。UUID Generatorでは最大100件を一括生成してCSVとしてコピーできるため、テストデータやモックIDを大量に作る場面に役立ちます。' },
      { title: 'UUID v1（タイムスタンプ）', body: 'UUID v1はMACアドレスと現在時刻（100ナノ秒精度）から生成されます。タイムスタンプが含まれるため生成順に並べることができますが、MACアドレスが含まれるためデバイスを特定できるプライバシー上の懸念があります。ログのトレースIDや時系列で並べたいIDに使えますが、MACアドレスを含まないUUID v4や、より安全でソート可能なUUID v7が代替として推奨されています。UUID v1のタイムスタンプ部分は1582年10月15日（グレゴリオ暦開始日）からの100ナノ秒単位の経過値で、Unixエポックとは異なります。このタイムスタンプ形式の特殊性もv7が登場した理由の一つです。' },
  ...
  'password-generator': {
    ...
    sections: [
      ...
      { title: 'パスワードのNGパターン', body: '辞書に載っている単語・誕生日・名前・連続した数字（123456・password・qwerty）は避けてください。これらはブルートフォース攻撃の辞書に最初に含まれます。同じパスワードを複数サービスで使い回すと、1つのサービスが漏洩したときに他のアカウントが芋づる式に乗っ取られるリスクがあります（クレデンシャルスタッフィング攻撃）。Have I Been Pwnedのような漏洩チェックサービスでは過去に流出したパスワードのハッシュデータベースと照合できます。また「複雑に見えるが実は規則的」なパターン（P@ssw0rd・Winter2024!）も辞書に含まれており、安全ではありません。パスワードジェネレーターで生成した真のランダムパスワードを使うことが最善策です。' },
      { title: 'パスフレーズの活用（デスクトップ版）', body: 'パスフレーズは「correct-horse-battery-staple」のように複数の英単語をつないだ長いパスワードです。ランダムな文字列より記憶しやすく、長さによる強度も十分に確保できます。デスクトップ版パスワードジェネレーターではパスフレーズ生成機能を搭載しています。4単語のパスフレーズは12〜16文字以上のランダム文字列と同等かそれ以上の強度を持ちます（エントロピー計算に基づく）。XKCD方式（xkcd.com/936）として広く知られており、Dicewareという手法で物理サイコロとワードリストから生成する本格的な実装もあります。マスターパスワードのように覚える必要があるパスワードにはパスフレーズが特に有効で、パスワードマネージャーの解除パスワードとしても推奨されます。' },
      { title: 'パスワードマネージャーとの組み合わせ', body: 'パスワードジェネレーターで生成した複雑なパスワードは、1Password・Bitwarden・KeePass・Dashlaneなどのパスワードマネージャーで保存するのが最善です。マスターパスワードだけを覚えればよく、各サービスごとに異なる強力なパスワードを使えます。パスワードマネージャーが内蔵するパスワードジェネレーターは自動入力と連携するため一番使いやすいですが、外部ツール（Password Generator）を使って生成し手動で登録する運用でも十分です。Bitwardenはオープンソースで無料プランが充実しており、セルフホストも可能なため企業利用にも向いています。重要なアカウントには必ず一意のパスワード＋二段階認証（TOTP）を組み合わせることがセキュリティの基本です。' },
    ],
  },
  ...
  'cron-builder': {
    ...
    sections: [
      { title: 'CRON式の5フィールド', body: 'CRON式は「分 時 日 月 曜日」の5つのフィールドで構成されます。例えば「0 9 * * 1-5」は「平日（月〜金）の毎朝9時0分」を意味します。*はすべての値、,は複数指定（0,30で0分と30分）、-は範囲（1-5で月〜金）、/はステップ値（*/15で15分ごと）を表します。Linuxの標準cronは5フィールドですが、Quartz（Javaのスケジューラ）では秒フィールドが先頭に追加された6〜7フィールド形式を使います。GitHub ActionsのCRON式もLinux標準の5フィールド形式ですがUTC基準です。CRON Builderではフィールドごとにビジュアルで設定でき、誤ったCRON式を入力するとリアルタイムにエラーを表示します。' },
      { title: 'よく使うCRON式パターン', body: '毎分: * * * * * / 毎時0分: 0 * * * * / 毎日午前0時: 0 0 * * * / 毎週月曜9時: 0 9 * * 1 / 毎月1日0時: 0 0 1 * * / 平日9〜18時の毎時: 0 9-18 * * 1-5。CRON Builderではこれらをプリセットとして用意しており、UIで選択・調整してCRON式を組み立てられます。月末日のCRON設定はL記号で表せますが（「0 0 L * *」= 毎月最終日）、対応していないスケジューラもあります。多くのクラウドサービス（AWS EventBridge・GCP Cloud Scheduler・Vercel cron）ではCRON式を設定でき、サーバーレス関数の定期実行やデータ集計ジョブの自動化に使われます。' },
  ...
  'base64-studio': {
    ...
    sections: [
      ...
      { title: 'Base64を使う主なシーン', body: 'JWTのHeader・Payloadはどちらも独自のBase64（URL-safe）でエンコードされています。メールの添付ファイル（MIME）もBase64で本文に埋め込まれます。REST APIでバイナリファイルをJSONに含めて送信するときにも使われます。Base64 Studioではテキスト・画像ファイル両方のエンコード/デコードに対応しています。Basic認証のAuthorizationヘッダーは「user:password」をBase64エンコードした文字列です（Authorization: Basic dXNlcjpwYXNz形式）。SSH公開鍵・TLS証明書（PEMファイル）もBase64エンコードされています。GitHubのSecrets・環境変数にバイナリファイルを格納する場合もBase64変換が使われます。' },
    ],
  },
  ...
  'gradient-deck': {
    ...
    sections: [
      ...
      { title: 'CSSグラデーションのパフォーマンスと注意点', body: 'CSSグラデーションは画像ファイルと異なりHTTPリクエストが不要です。ただしブラウザの再描画コストがあるため、アニメーションの背景として使う場合はtransformやopacityアニメーションより重くなることがあります。Gradient Editorでは生成したCSSをワンクリックでコピーでき、プリセット集から参考にして調整する使い方もできます。' },
    ],
  },
  ...
  'text-layout': {
    ...
    sections: [
      ...
      { title: 'A/B比較モードの活用', body: '2つのテキスト設定を左右に並べて比較するA/Bモード（デスクトップ版）を使うと、「現在の設定」と「新しい設定」を視覚的に比較できます。フォントのチョイスで迷ったとき、行間の広い版と狭い版を並べて選択するといった使い方が効率的です。デザインの意思決定を直感と感覚だけで行うのではなく、具体的な数値設定とビジュアルの対応関係を確認しながら進めることで、チームへの説明やドキュメント化もしやすくなります。最終的に採用した設定値はCSS変数として保存し、デザインシステムに組み込むと一貫性が保てます。' },
    ],
  },
  ...
  'text-deck': {
    ...
    sections: [
      ...
      { title: 'タグによる分類と検索', body: 'Snippet Launcherではスニペットにタグを付けて管理し、タグフィルタリングで目的のスニペットをすばやく見つけられます。タイトルと本文のキーワード検索も組み合わせることで、数十件のスニペットの中から1〜2秒で目的のものを取り出せます。開発・メール・SNS投稿など用途別にタグを付けておくのがおすすめです。' },
      { title: 'グローバルホットキーで瞬時に呼び出す（デスクトップ版）', body: 'デスクトップ版ではCtrl+Shift+Dで他のアプリを操作中でもSnippet Launcherを前面に呼び出せます。コーディング中にIDEを離れることなくスニペットをコピーしてすぐに戻れるため、作業フローが途切れません。システムトレイに常駐するため、Windowsの起動と同時に自動起動する設定が可能です。' },
      { title: 'スニペットファイルへの保存と共有', body: 'ブラウザ版はlocalStorageに保存しますが、デスクトップ版はファイルに保存して件数無制限で管理できます。スニペットファイルをDropboxやGoogle Driveに保存しておけば複数のPCで同じスニペットを参照できます。JSONファイル形式でエクスポート・インポートができるため、バックアップや新PC移行も手間なく行えます。チームでよく使うコードスニペットや回答テンプレートを共有ファイルとして管理すれば、全員が最新のスニペットを参照できるチーム用スニペット管理として活用できます。' },
    ],
  },
  ...
  'ai-prompt-manager': {
    ...
    sections: [
      ...
      { title: 'プロンプトの再利用と実験管理', body: '気に入った画像を生成できたプロンプトを保存しておくことで、後から同じスタイルで別の構図の画像を生成しやすくなります。タグをカテゴリ別にON/OFFすることで「このキャラクターで背景だけ変える」「服装だけ変えて試す」といった実験管理ができます。どのタグの組み合わせが高品質な出力につながったかを記録することで、自分だけのプロンプトエンジニアリングのノウハウが蓄積されていきます。LoRA・ControlNet・CFGスケールなどの追加パラメータもメモとして一緒に保存しておくと再現性が上がります。' },
    ],
  },
  ...
  'mockup-builder': {
    ...
    sections: [
      ...
      { title: 'デスクトップ版の全サイズ一括生成', body: 'デスクトップ版MockupBuilderではiPhone 6.7/6.5/5.5インチ+Android用の全サイズを一括でZIP生成できます。App Store申請時に必要な全サイズを1クリックで揃えられるため、複数サイズへの書き出し作業が不要になります。出力先フォルダを指定してバッチ処理できるため、スクリーンショットを更新するたびに全サイズを手作業で作り直す手間がなくなります。アプリのアップデートリリースのたびに発生するスクリーンショット更新作業を大幅に効率化できます。' },
    ],
  },
  ...
  'sheet-studio': {
    ...
    sections: [
      ...
      { title: 'アニメーションプレビューで動作確認（デスクトップ版）', body: 'デスクトップ版Sheet Studioではパックしたスプライトシートをアニメーションプレビューで確認できます。FPS（フレームレート）を調整してキャラクターアニメーションの速度感を確認しながらスプライトの並び順を最適化できます。一般的なキャラクターのアニメーションは4〜12FPSが多く、滑らかな動きには12〜24FPSが必要です。プレビューで確認しながらFPSとフレーム数の最適なバランスを見つけることで、ゲームのパフォーマンスとビジュアル品質を両立させることができます。Godot・Unity・Phaser.jsなど主要なゲームエンジンへのインポート手順も確認できます。' },
    ],
  },
  ...
  'diff-viewer': {
    ...
    sections: [
      ...
      { title: '実際の活用シーン', body: '設定ファイルを変更する前後の比較、コードレビューの事前確認、翻訳作業で原文とチェック済み訳文の比較、APIのレスポンスが変わっていないか確認、ドキュメントの改訂差分を確認するなど、テキスト系の作業なら何にでも使えます。特にデプロイ前に.envや設定ファイルを変更した際、どこを変えたか確認する用途で重宝します。コードのリファクタリング後にロジックが変わっていないか検証するときにも、before/afterを貼り付けて即差分チェックができます。テキストエディタのdiffプラグインと異なり、環境に依存せずブラウザだけで完結します。' },
    ],
  },
  ...
  'border-radius-generator': {
    ...
    sections: [
      ...
      { title: 'ピル形状・タグ形状の作り方', body: 'border-radius: 999px（またはborder-radius: 9999px）のような非常に大きな値を指定すると、幅に関わらず半円の端を持つピル形状になります。バッジ・ラベル・タグUIによく使われます。border-radius: 0 で角丸を削除してシャープな角にできます。ボタンのデザインでは丸みのある形（border-radius: 8px程度）・ピル形状（border-radius: 9999px）・角ばった形（border-radius: 0）という3種のスタイルが基本バリエーションです。CSSのトランジション（transition: border-radius 0.3s）を付けると、ホバー時に角丸が変化するアニメーションも実装できます。' },
  ...
  'color-shade-generator': {
    ...
    sections: [
      { title: 'Tint・Shade・Toneとは', body: 'Tintは色に白を混ぜて明るくした色です。背景色・セカンダリボタン・ホバー状態に使われます。Shadeは色に黒を混ぜて暗くした色です。テキスト・ボーダー・フォーカス状態に活用されます。Toneはグレーを混ぜて彩度を下げた色で、落ち着いた印象が必要な場面に向いています。デザインシステムではこの3方向を組み合わせて1つのブランドカラーから10段階以上のカラースケールを作るのが一般的です。明るすぎるTintは白とのコントラストが下がりアクセシビリティ問題を起こしやすいため、WCAG基準（4.5:1以上）で確認しながらスケールを調整するのがベストプラクティスです。' },
  ...
  'css-mask-generator': {
    ...
    sections: [
      ...
      { title: '-webkit-プレフィックスの必要性', body: 'CSS Maskの実装状況はブラウザによって異なります。特にSafari（WebKit系）では-webkit-mask-imageが必要です。CSS Mask Generatorはmask-imageと-webkit-mask-imageの両方を出力するため、Safari・Chrome・Firefoxを含む主要ブラウザに対応したコードが得られます。Can I Useで確認するとCSS Masking Propertiesは主要ブラウザで90%以上サポートされていますが、ベンダープレフィックスなしのmask-imageはSafari 15.4以降から対応のため、古いiOSデバイスへの対応が必要な場合は-webkit-プレフィックス付きとの両方記述が依然として必要です。' },
      { title: '複数マスクの重ね合わせ', body: 'CSS maskは複数のマスクをカンマ区切りで重ねられます。横方向と縦方向のグラデーションマスクを組み合わせて四辺フェードを作れます。mask-composite: excludeなどのコンポジット操作でマスクの合成方法を変えることも可能です。CSS Mask Generatorは最大3つのマスクレイヤーに対応しています。mask-compositeにはadd（合算）・subtract（減算）・intersect（積）・exclude（XOR）の4モードがあり、複雑な切り抜き表現を作れます。mask-sizeとmask-positionを組み合わせると、タイル状に繰り返すパターンマスクやアニメーションするマスクも実装できます。' },
    ],
  },
  ...
  'curl-to-code': {
    ...
    sections: [
      ...
      { title: 'Python requestsへの変換', body: 'curlをPythonに変換するとrequests.get() / requests.post()などの形式になります。-Hオプションはheaders辞書に、-dはjson=またはdata=引数に対応します。import requestsが必要で、response.json()でレスポンスをPythonオブジェクトとして取得できます。Basic認証（-u user:pass）はrequests.get(url, auth=("user", "pass"))に変換されます。-kオプション（SSL検証無効）はverify=Falseに対応します。セッションを使い回す場合はrequests.Session()を使いヘッダーをsession.headers.update()で一括設定するとよりPythonicなコードになります。' },
  ...
  'color-token-generator': {
    ...
    sections: [
      ...
      { title: 'Tailwindカスタムカラーへの転用', body: 'Color Token GeneratorはTailwind CSS形式での出力に対応しています。生成したパレットをtailwind.config.jsのtheme.extend.colorsにコピーするだけでprimary-500・primary-600などのカスタムクラスが使えるようになります。Tailwind v4ではtailwind.config.jsの代わりに@theme { --color-primary-500: #3b82f6; }のようにCSSファイルに直接トークンを定義する方式に移行しました。Color Token Generatorが出力するCSS変数形式はそのままTailwind v4のテーマ定義として使えます。' },
  ...
  'favicon-generator': {
    ...
    sections: [
      { title: 'faviconのファイル形式と推奨サイズ', body: '従来のfavicon.icoは16×16・32×32の複数サイズを1ファイルに含めます。現代的なWebサイトではSVG（favicon.svg）・PNG・WebPが使えます。apple-touch-icon（180×180px）はiOSのホーム画面追加時に使われます。Androidでは192×192・512×512pxのPNGが推奨されます。SVGのfaviconはDark Mode対応が可能で、@media (prefers-color-scheme: dark) { ... } をSVG内に書くとライト/ダーク自動切り替えができます。マニフェストファイル（manifest.webmanifest）でPWA向けのアイコンセットをまとめて定義できます。' },
  ...
  'json-csv-converter': {
    ...
    sections: [
      ...
      { title: 'JSON配列からCSVへの変換', body: '最も単純なケースはオブジェクトの配列（[{"name":"田中","age":28},...]）です。この場合、各オブジェクトの1件が1行、キーがCSVのヘッダー行になります。JSON → CSV Converterでは配列を貼り付けるとヘッダー行が自動抽出され、テーブルプレビューで確認しながら変換できます。ネストしたオブジェクトはフラット化（address.city）またはJSON文字列として1セルに格納するかを選べます。数値・booleanは型を保持した状態でCSV変換すると一部のツールで型推論されます。文字コードはUTF-8またはShift_JISを選択でき、Excelで開く場合はShift_JIS（またはBOM付きUTF-8）が文字化けしにくいです。' },
  ...
  'date-calculator': {
    ...
    sections: [
      ...
      { title: '営業日カウント', body: '営業日計算では土曜・日曜を除外します。日本では祝日も除外する必要があり、祝日データを別途管理する必要があります。Date Calculatorの営業日モードでは土日を自動除外してカウントします。「5営業日以内に発送」のような納期計算や「20営業日後の期限日」の算出に使えます。開始日を含めるか含めないかがビジネスルールによって異なるため、確認が必要です。ライブラリとしてはdate-fnsのaddBusinessDays、moment.jsのisoWeekday比較などが使われます。国際的なアプリでは国ごとに祝日が異なるため、holiday-jpなどの国別祝日ライブラリを組み合わせて使います。' },
  ...
  'cidr-calculator': {
    ...
    sections: [
      ...
      { title: 'プライベートIPアドレスの範囲', body: 'RFC 1918で定義されたプライベートIPアドレス範囲は①10.0.0.0/8（10.x.x.x）・②172.16.0.0/12（172.16.x.x〜172.31.x.x）・③192.168.0.0/16（192.168.x.x）の3つです。インターネットには直接ルーティングされず、社内LAN・クラウドの内部ネットワーク・自宅NATルーターで使われます。AWS VPCでは10.0.0.0/16などの大きなブロックを使い、サブネットを細かく分割するのが一般的です。リンクローカルアドレス（169.254.0.0/16）はDHCP失敗時に自動割り当てられ、クラウド環境ではインスタンスメタデータのエンドポイント（169.254.169.254）としても使われます。' },
  ...
  'user-agent-parser': {
    ...
    sections: [
      ...
      { title: 'ブラウザとレンダリングエンジンの識別', body: 'レンダリングエンジンはChromeとEdgeがBlink（WebKit派生）、FirefoxがGecko、SafariがWebKit、旧IEがTridentを使います。UAのAppleWebKitの有無でWebKitベースか判断しますが、ChromeもAppleWebKitを含むため、Chromeの識別には続くChrome/バージョン番号を確認します。EdgeはChromiumベースになった（Chromium Edge/v79以降）ためBlinkを使い、UAにEdg/バージョンを含みます。JavaScriptのnavigator.userAgentで取得できますが、Sec-CH-UA（User-Agent Client Hints）という新しい仕様では、より信頼性の高いブラウザ情報をHTTPヘッダーで取得できます。' },
  ...
  'media-query-builder': {
    ...
    sections: [
      { title: 'メディアクエリの基本構文', body: '@media (max-width: 768px) { ... } のように@mediaに続いて条件を書き、中括弧内にそのとき適用するCSSを書きます。min-width（この幅以上）・max-width（この幅以下）が最もよく使われます。複数条件はandで結合します（@media (min-width: 768px) and (max-width: 1024px)）。CSS Media Queries Level 4では@media (width >= 768px)のような範囲記法が追加され、より直感的な書き方が可能になっています。not・orも使えます。@mediaの前には改行を入れてネストを視覚的に明確にするのが慣例です。' },
  ...
  'sql-formatter': {
    ...
    sections: [
      { title: 'SQLフォーマットの慣例', body: 'SELECT・FROM・WHERE・JOIN・GROUP BY・ORDER BY・HAVINGなどのSQLキーワードは大文字で書くのが一般的です。列名・テーブル名・エイリアスは小文字またはスネークケースが多く使われます。各キーワードを新しい行の先頭に揃えるとSELECT節・FROM節・WHERE節が明確になります。チームによってはすべて小文字で統一するスタイルもあり、重要なのは一貫性です。SQLの行末セミコロンはサーバーへの送信時には不要ですが、複数クエリを1つのファイルにまとめる際は区切りとして必要です。コメントは-- 一行コメントまたは/* 複数行コメント */で追記できます。' },
      { title: 'インデントと複雑なクエリの整形', body: 'サブクエリ・CASE式・WITH句（CTE）などはインデントで入れ子構造を表現します。JOINが複数ある場合はON条件をJOINと同じ行またはインデントして続けます。長いWHERE条件はAND/ORで改行して揃えると読みやすくなります。SQL Formatterはこれらのフォーマット規則を自動適用します。CTEはWITH句でサブクエリに名前を付ける機能で、複雑なクエリを読みやすい段階に分解できます。CASE WHEN ... THEN ... ELSE ... ENDは縦に揃えると各条件が読みやすくなります。ORDER BY節は最後に配置するのが慣例です。' },
      { title: 'MySQL・PostgreSQL・SQLiteの方言', body: 'SQLには標準SQLの他にデータベースごとの方言があります。文字列結合はMySQLがCONCAT()・PostgreSQLが||・SQLiteが両方対応します。日付関数はデータベースによって大きく異なります。SQL FormatterはMySQL・PostgreSQL・SQLiteの構文を意識したフォーマットに対応しています。現在時刻の取得はMySQLがNOW()・PostgreSQLがNOW()またはCURRENT_TIMESTAMP・SQLiteがdatetime(\'now\')です。LIMIT句はMySQL・SQLiteがLIMIT/OFFSET形式、PostgreSQLも同様ですがFETCH FIRST N ROWS ONLY（SQL標準）も使えます。MySQL固有の文法（REPLACE INTO、INSERT ... ON DUPLICATE KEY UPDATE）はPostgreSQLではINSERT ... ON CONFLICTで代替します。' },
      { title: 'パフォーマンスとインデックスの基礎', body: 'クエリのパフォーマンスはインデックスの有無に大きく依存します。WHERE句の条件に使うカラム・JOIN条件・ORDER BYのカラムにはインデックスを作成することを検討します。EXPLAIN（MySQL・PostgreSQL）でクエリの実行計画を確認し、フルスキャンが発生していないかを確認します。整形されたSQLはEXPLAINの結果と対応づけて理解しやすくなります。PostgreSQLではEXPLAIN ANALYZEで実際の実行時間を計測できます。インデックスは読み取りを高速化しますが、書き込み時のオーバーヘッドが発生するためやみくもに追加するのは避けましょう。N+1問題はEager Loading（JOINやSELECT ... IN）で解消し、不必要な複数クエリを1回に減らすのがORMパフォーマンス最適化の基本です。' },
    ],
  },
  ...
  'cookie-inspector': {
    ...
    sections: [
      { title: 'Set-Cookieヘッダーの構文', body: 'Set-Cookie: name=value; Expires=日時; Max-Age=秒数; Path=/; Domain=.example.com; Secure; HttpOnly; SameSite=Strict の形式です。ExpiresとMax-Ageの両方が指定された場合はMax-Ageが優先されます。どちらもなければセッションクッキー（ブラウザを閉じると削除される一時クッキー）になります。名前と値はURLエンコードが推奨で、セミコロン・カンマ・スペースはエンコードが必須です。複数のクッキーを設定するには、Set-Cookieヘッダーを複数回指定します（1つのSet-Cookieヘッダーで複数クッキーを設定することはできません）。ブラウザのDevToolsのApplicationタブでCookieの全属性を確認・編集できます。' },
  ...
  'robots-txt-generator': {
    ...
    sections: [
      ...
      { title: 'Crawl-delay でクロール頻度を制御', body: 'Crawl-delay: 10のように秒数を指定すると、クローラーがページ間でその秒数だけ待機します。サーバー負荷が高い場合に有効ですが、Googleはこのディレクティブを現在無視しています（Google Search Consoleのクロールレート設定を使う）。Bingは対応しています。過剰なCrawl-delayはインデックス速度の低下につながります。Googleのクロールレートを制限したい場合は、Google Search ConsoleのCrawl Rate Limiterを使うのが唯一の公式な方法です。サーバーがクロールによって503を返す場合、Googleは自動的にクロール頻度を下げる仕組みも持っています。robots.txtの変更はGoogleが検出するまで数日かかる場合があります。' },
  ...
  'mime-type-lookup': {
    ...
    sections: [
      { title: 'MIMEタイプの構造', body: 'MIMEタイプは「タイプ/サブタイプ」の形式で記述します（例: image/png、application/json、text/html）。タイプにはtext/image/audio/video/application/multipartなどがあり、サブタイプで具体的な形式を示します。+json・+xmlなど構造化サフィックスを使うこともあります（application/vnd.api+json）。vendorプレフィックス（vnd.）はベンダー固有のフォーマットを示し、x-プレフィックスは非標準の実験的フォーマットに使われます。charsetパラメーターを付加してエンコーディングを指定できます（text/html; charset=UTF-8）。IANAがMIMEタイプの登録を管理しており、公式のMIMEタイプ一覧はIANAのサイトで参照できます。' },
      { title: 'Content-Typeを正しく設定しないとどうなるか', body: 'HTMLファイルをtext/plainで返すとブラウザがそのまま文字列として表示してしまいます。JavaScriptをtext/htmlで返すと実行されません。APIのJSONレスポンスにapplication/jsonを設定しないとクライアントライブラリが自動パースしない場合があります。セキュリティ面では、ブラウザのMIMEスニッフィング（Content-Typeを無視して実際の内容から形式を推測する機能）を悪用したXSS攻撃が存在します。これを防ぐにはX-Content-Type-Options: nosniffヘッダーを設定します。SVGをimage/svg+xmlとして返す場合もXSSリスクがあるため、ユーザーアップロードのSVGはtext/plainやapplication/octet-streamで返すのが安全です。' },
      { title: 'よく間違えやすいMIMEタイプ', body: 'SVGはimage/svg+xml（image/svgではない）。JavaScriptはapplication/javascript（text/javascriptは旧仕様だがまだ動く）。JSONはapplication/json。フォントはfont/woff2またはfont/woff。CSVはtext/csvだがExcel（.xlsx）はapplication/vnd.openxmlformats-officedocument.spreadsheetml.sheet。Webpはimage/webp。AVIFはimage/avif。PDFはapplication/pdf。MP4はvideo/mp4。MP3はaudio/mpeg。これらの違いを正確に知るためにMIMEタイプ検索ツールが役立ちます。サーバー設定（NginxのMIMEタイプマップ・ApacheのAddTypeディレクティブ）でも確認・追加できます。' },
  ...
  'openapi-viewer': {
    ...
    sections: [
      ...
      { title: '$refと再利用コンポーネントの活用', body: '$refは別定義を参照する仕組みで「$ref: "#/components/schemas/User"」のように使います。スキーマ・パラメーター・レスポンスを一度定義して複数箇所から参照でき、仕様書のDRY原則を守れます。OpenAPI Viewerは$refを解決して実際のスキーマ内容を展開表示するため、定義元を行き来する手間が不要です。componentsオブジェクトにはschemas・responses・parameters・requestBodies・headers・securitySchemesなどのセクションがあり、それぞれ再利用可能な定義を置けます。外部ファイルへの$refも有効で、$ref: "./schemas/user.yaml"のように書くと大規模仕様書を複数ファイルに分割管理できます。' },
      { title: 'パラメーターの種類（in: の値）', body: 'path（URLパス中の変数：/users/{id}）、query（?key=value）、header（リクエストヘッダー）、cookie（Cookieヘッダー）の4種類があります。それぞれ異なるバリデーションルールがあり、requiredフィールドで必須かどうかを示します。body（Swagger 2.0）はOpenAPI 3.xではrequestBodyに移動しました。styleプロパティでパラメーターのシリアライズ形式（form・simple・matrix・label・deepObject）を制御でき、配列やオブジェクトをクエリパラメーターとして渡す際に使います。explodeプロパティでarray要素を?tags=a&tags=bのように展開するかtags=a,bのように連結するかを決められます。' },
      { title: 'APIデザインのベストプラクティス', body: 'パスはリソース名（複数形の名詞）をスラッシュで階層化し、動詞はHTTPメソッドで表します（GET /users, POST /users, DELETE /users/{id}）。レスポンスには200/201/400/401/404/500を適切に定義します。descriptionにはサンプル値と共にエラーレスポンスの例を含めると実装者にとって親切です。exampleやexamplesフィールドを使うと実際のリクエスト/レスポンスの例を仕様書に埋め込めます。AuthorizationヘッダーはsecuritySchemesでベアラートークン・APIキー・OAuth 2.0などを定義し、各エンドポイントのsecurityで適用します。仕様書はopenapi-generatorで各言語のクライアントSDKを自動生成するためにも使えます。' },
    ],
  },

  'container-query-builder': {
    heading: 'CSS Container Queriesとは — コンポーネントベースのレスポンシブデザイン',
    intro: 'CSS Container Queriesは、ビューポートではなくコンテナ要素のサイズに基づいてスタイルを切り替える新しいCSS機能です。メディアクエリが「画面サイズ」に反応するのに対し、コンテナクエリは「コンポーネントのサイズ」に反応します。',
    sections: [
      { title: 'メディアクエリとの違い', body: '`@media (min-width: 768px)` はブラウザのビューポート幅に反応します。一方 `@container (min-width: 400px)` は指定したコンテナ要素の幅に反応します。同じコンポーネントでもサイドバー内に置いたときとメインエリアに置いたときで異なるスタイルを適用できるため、真にコンポーネントベースのレスポンシブデザインが実現できます。' },
      { title: 'container-typeの設定', body: '`container-type: inline-size` は横幅（インライン方向）のクエリのみを有効にします。`container-type: size` は縦横両方のクエリを有効にしますが、コンテナ要素に明示的な高さが必要です。ほとんどのユースケースでは `inline-size` で十分です。`container-name` でコンテナに名前を付けると、ネストしたコンテナを明示的に指定して @container クエリを書けます。' },
      { title: '実用的なパターン', body: 'カードコンポーネントが400px以上のコンテナ内にある場合は横並びレイアウト、それ以下では縦積みレイアウトに切り替えるのが典型的な使い方です。ナビゲーションメニュー・価格カード・メディアオブジェクトなどをコンテナクエリでレスポンシブにすることで、グリッドシステムやブレークポイントへの依存を減らせます。たとえばサイドバーに配置されたウィジェットは幅が狭いため縦積み表示になり、メインコンテンツエリアに移動すると自動で横並びに切り替わります。これをメディアクエリだけで実現するには画面幅とレイアウト両方の条件が必要で複雑になりますが、コンテナクエリならコンポーネント自身のサイズだけで判断できるためシンプルに保てます。' },
      { title: 'ブラウザサポート', body: 'Container Queriesは2023年以降の主要ブラウザ（Chrome 105+, Firefox 110+, Safari 16+）でサポートされています。IE・古いEdgeは非対応ですが、現代のWebアプリケーション開発では実用的に使えます。Chromeで最初に実装されてから急速に他ブラウザにも普及し、2024年現在ではグローバル対応率90%以上で安心して使える機能です。非対応ブラウザ向けのフォールバックとして@supportsで分岐するか、PostCSSプラグインで変換する方法があります。Style Queriesと呼ばれる拡張機能（CSS変数の値に基づくクエリ）も一部ブラウザで実装が進んでいます。' },
    ],
  },

  'json-to-sql': {
    heading: 'JSONからSQLを生成する — 型推論とINSERT文の自動化',
    intro: 'APIレスポンスやCSVから変換したJSONデータをデータベースに投入したい場面は多くあります。手動でCREATE TABLEとINSERT文を書くのは手間がかかりますが、このツールを使えばJSONを貼るだけで自動生成できます。',
    sections: [
      { title: '型推論の仕組み', body: '各カラムの値を全行スキャンしてSQL型を決定します。すべての値が整数なら INTEGER、浮動小数点数があれば REAL、文字列があれば TEXT、true/falseのみなら BOOLEAN として推論します。型が混在する場合は最も広い型（TEXT）が選ばれます。null値を含む行があるカラムはNULL許容（NOT NULL なし）になります。日付・時刻のISO 8601文字列（2024-01-15など）はデータベースによってTIMESTAMP型を推論することもあります。カラム名にidやIdが含まれる場合は自動でPRIMARY KEY候補として扱うことがあります。生成後のCREATE TABLE文はそのままSQLクライアントに貼り付けて実行できます。' },
      { title: '方言ごとの違い', body: 'PostgreSQLとSQLiteはダブルクォートでカラム名・テーブル名を囲みますが、MySQLはバッククォートを使います。BOOLEAN型はPostgreSQL/SQLiteではTRUE/FALSEリテラルを使いますが、MySQLはTINYINT(1)で0/1で格納します。浮動小数点数はMySQLではDOUBLEが一般的なので自動的に変換されます。自動採番のPRIMARY KEYはPostgreSQLではSERIALまたはGENERATED ALWAYS AS IDENTITY、MySQLではAUTO_INCREMENT、SQLiteではINTEGER PRIMARY KEY（rowid のエイリアス）でそれぞれ定義方法が異なります。文字列の最大長もPostgreSQL/MySQLではVARCHAR(255)など指定しますが、SQLiteは長さ制限がなくTEXTのみで事足ります。' },
      { title: 'スキーマ名の活用', body: 'PostgreSQLでは`public.users`のようにスキーマ名を指定してテーブルを整理します。スキーマ名フィールドを入力すると`"schema"."table"`形式でCREATE TABLEとINSERT文が生成されます。MySQLの場合はデータベース名を指定することで`database`.`table`形式になります。PostgreSQLのスキーマはデータベース内の名前空間で、テナント分離（テナントごとにスキーマを切り替える）やモジュール分割（auth・billing・notificationなど）に活用されます。search_pathを設定すれば毎回スキーマ名を書かずにアクセスできます。SQLiteにはスキーマ概念がなく、ATTACHコマンドで複数データベースを接続する方法で代替します。' },
      { title: '大量データの投入', body: '生成されたINSERT文は全行を1つのINSERT INTO ... VALUES(...),(...),(...)形式（マルチロウINSERT）ではなく、行ごとに分けています。これにより1行ずつ実行して部分的にデバッグしやすくなります。大量データの本番投入にはPostgreSQLの\\COPYやMySQLのLOAD DATA INFILEがCSVを直接読み込めるためより高速です。PostgreSQLでは\COPY users FROM \'data.csv\' CSV HEADERで数百万件のインポートも数秒で完了します。Node.jsではpg-copystreamsライブラリ、Pythonではpsycopgのcopy_expertメソッドを使って大量データをストリーミングINSERTできます。' },
    ],
  },

  'totp-generator': {
    heading: 'TOTP（RFC 6238）とは — 2段階認証の仕組みを理解する',
    intro: 'TOTP（Time-based One-Time Password）はRFC 6238で定義されたワンタイムパスワードのアルゴリズムです。Google AuthenticatorやAuthyなどの2FAアプリはこの標準を使っています。シークレットキーと現在時刻からHMAC-SHA1で6桁のコードを生成します。',
    sections: [
      { title: 'TOTPの生成ステップ', body: '① シークレットキー（Base32）をバイト列にデコード。② 現在のUnix時刻を30秒で割って切り捨てた値（カウンター）を8バイトのbig-endianで表現。③ HMAC-SHA1でシークレットとカウンターを計算。④ 結果の最後のバイトの下位4ビットをオフセットとして4バイトを取り出し、最上位ビットをクリアして10の6乗で割った余りが6桁コードになります。' },
      { title: 'Base32エンコードとは', body: 'Base32はアルファベット大文字A-Zと数字2-7の32文字を使うエンコード方式です（RFC 4648）。Base64と違い数字0・1や大文字と見間違えやすい文字を避けているため、手入力での誤りが起きにくいという特徴があります。TOTPのシークレットキーの配布にBase32が使われる理由です。QRコードでスキャンする場合も内部的にはBase32文字列が埋め込まれています。' },
      { title: '前後のコードが有効な理由', body: '時刻のずれ（クロックスキュー）に対応するため、多くの2FA実装では現在のコードに加えて前後1ステップ（±30秒）のコードも有効と見なします。このツールでは前・現在・次の3コードを表示するため、時刻がわずかにずれている環境でのデバッグに役立ちます。RFC 6238では最大±1ステップ（60秒の許容誤差）を推奨しています。許容ウィンドウを広げすぎると攻撃者がコードを盗み見てから使う猶予が増えるため、サーバーはNTP同期を維持して時刻ずれを最小化するのが本来の対策です。ユーザーのデバイスと認証サーバーの時刻が30秒以上ずれているとコードが一致しない問題が発生するため、スマートフォンの自動時刻同期設定は有効にしておくことが推奨されます。' },
      { title: 'OTP実装のデバッグ用途', body: 'アプリにTOTP認証を組み込む際、生成したコードが正しいか確認するためにこのツールが使えます。同じBase32シークレットを使って、実装したコードとこのツールの出力が一致していれば正しく実装されています。WebCrypto APIのHMAC-SHA1をブラウザで直接実行するため、通信なしで安全に確認できます。サーバー側の実装テストには、テスト用の固定シークレット（例：JBSWY3DPEHPK3PXP）と固定時刻でRFC 6238のテストベクターを使って検証するのが確実です。Node.jsではspeakeasyやotplibライブラリが標準的なTOTP実装を提供しており、これらとの比較検証にも使えます。' },
    ],
  },

  'html-to-markdown': {
    heading: 'HTMLをMarkdownに変換する理由と使い方',
    intro: 'WebページのHTMLをMarkdownに変換したい場面は多くあります。ブログ記事の移行、ドキュメントのコピー、CMS間のコンテンツ移植など。このツールはブラウザ内で完結するHTML→Markdown変換を提供します。',
    sections: [
      { title: 'MarkdownとHTMLの対応関係', body: 'MarkdownはHTMLを人間が読みやすい記法で書くための軽量マークアップ言語です。`<h1>`は`# 見出し`、`<strong>`は`**太字**`、`<a href="url">テキスト`は`[テキスト](url)`に対応します。GitHubやZennなどのプラットフォームはGFM（GitHub Flavored Markdown）を採用しており、テーブル記法も追加されています。' },
      { title: 'GFMテーブルの変換', body: 'HTML の `<table>` タグは GFM のパイプ記法テーブルに変換されます。`| ヘッダー1 | ヘッダー2 |` の形式でヘッダー行、`| --- | --- |` でセパレータ行、その後に各データ行が続きます。ヘッダーセルが `<th>` でなく `<td>` の場合でも最初の行はヘッダーとして扱います。`| :--- |` で左揃え、`| :---: |` で中央揃え、`| ---: |` で右揃えを指定できます。セル内にパイプ文字（|）が含まれる場合は`\|`のようにエスケープが必要です。複雑なHTMLテーブル（セル結合・入れ子テーブルなど）はMarkdownのパイプ記法では表現できないため、そのままHTMLタグとして出力されます。' },
      { title: 'コードブロックの変換', body: '`<pre><code>` は ``` で囲まれたコードフェンスに変換されます。`<code class="language-javascript">` のようにクラスに言語名が含まれている場合は、そのままフェンスの言語ヒント（` ```javascript `）として出力されます。インラインの `<code>` はバッククォートで囲まれたインラインコードになります。' },
      { title: 'ネストしたリストの扱い', body: '`<ul>` と `<ol>` のネストは、インデント（2スペース）で表現されます。`- 親項目` の下に `  - 子項目` が続く形式です。`<li>` の中にさらに `<ul>` や `<ol>` がある場合も正しく再帰的に変換されます。`<ol>` の場合は自動で番号が振られ、`1. 項目1`・`2. 項目2` の形式になります。`<li>` 内にブロック要素（段落・コードブロックなど）がある場合は、インデントを維持したまま変換されます。`<dl>`（定義リスト）はMarkdownに直接対応する記法がないため、HTMLタグのままか`**用語**`+改行の近似形式で出力されます。' },
    ],
  },
  ...
  'json-merger': {
    ...
    sections: [
      ...
      { title: '設定ファイルの階層マージパターン', body: 'Node.jsアプリでよく使われるパターンとして、default.json → {NODE_ENV}.json → local.json の順にDeep Mergeすることで環境別設定を実現します。左側（優先度低）から右側（優先度高）の順でマージし、後のファイルの値が優先されます。このツールも同じ「後スロット優先」のマージ順を採用しています。このパターンはconfigや@nestjs/configライブラリが標準で採用しています。Kubernetesのパッチ（kubectl patch）もDeep Mergeを使っており、差分のみ指定して既存マニフェストに適用できます。TypeScriptのObject.assign()やlodash.mergeIsEqual()でも同様のパターンを実装できます。' },
  ...
  'svg-optimizer': {
    heading: 'SVGを最適化してファイルサイズを削減する方法',
    intro: 'デザインツールで書き出したSVGには、エディタのメタデータや冗長な小数、不要な空白が多く含まれ、ファイルサイズが必要以上に大きくなりがちです。SVG Optimizerはこうした不要データを除去し、表示に影響しない範囲でSVGを軽量化します。Web表示の高速化やアイコンのインライン化に役立ちます。',
    sections: [
      { title: 'SVGが重くなる原因', body: 'IllustratorやInkscapeなどで書き出したSVGには、編集用のメタデータ（<metadata>、inkscape:*・sodipodi:* 属性、名前空間宣言）、XML宣言、コメント、そして小数点以下が長い座標値が含まれます。これらは画面表示には不要ですが、そのままだとファイルサイズを大きくし、HTMLにインライン展開したときにコードを読みにくくします。特にアイコンを大量に使うサイトでは、この差の積み重ねが読み込み速度に効いてきます。' },
      { title: '安全に削減できるもの / 慎重に扱うもの', body: 'コメント・XML宣言・DOCTYPE・<metadata>・エディタ由来の属性は、表示に影響しないため安全に削除できます。一方、数値の丸めは桁を落としすぎると細かい形状がわずかにズレることがあります。アイコンなど小さく表示するSVGは2桁程度で十分ですが、大きく表示する複雑なパスは3〜4桁にすると安心です。width/height や viewBox は表示サイズに関わるため、SVG Optimizerでは削除しません。' },
      { title: '最適化後は必ずプレビューで確認する', body: '最適化はコード上の削減なので、見た目が変わっていないかの確認が重要です。SVG Optimizerは最適化後のSVGをその場でプレビュー表示するため、形状や色が崩れていないかをすぐ確認できます。もし崩れる場合は「数値を丸める」の桁数を増やすか、その工程をオフにしてください。処理はすべてブラウザ内で行われ、SVGは外部に送信されません。' },
      { title: 'Webでの活用（インラインSVG・アイコン）', body: '最適化したSVGは、<img>やCSSのbackgroundとして使うほか、HTMLに直接インライン展開してCSSで色やサイズを制御する使い方が効果的です。特にアイコンはインラインSVGにするとHTTPリクエストが減り、fillにcurrentColorを指定すれば文字色に追従させられます。生成したコードはReactコンポーネントに変換してそのまま使うこともできます。' },
    ],
  },
