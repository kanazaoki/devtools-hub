...
export const seoContent: Record<string, SeoArticleData> = {
  ...
  'container-query-builder': {
    ...
    sections: [
      ...
      { title: '実用的なパターン', body: 'カードコンポーネントが400px以上のコンテナ内にある場合は横並びレイアウト、それ以下では縦積みレイアウトに切り替えるのが典型的な使い方です。ナビゲーションメニュー・価格カード・メディアオブジェクトなどをコンテナクエリでレスポンシブにすることで、グリッドシステムやブレークポイントへの依存を減らせます。' },
      { title: 'ブラウザサポート', body: 'Container Queriesは2023年以降の主要ブラウザ（Chrome 105+, Firefox 110+, Safari 16+）でサポートされています。IE・古いEdgeは非対応ですが、現代のWebアプリケーション開発では実用的に使えます。Chromeで最初に実装されてから急速に他ブラウザにも普及し、2024年現在では安心して使える機能です。' },
    ],
  },
  ...
  'json-to-sql': {
    ...
    sections: [
      { title: '型推論の仕組み', body: '各カラムの値を全行スキャンしてSQL型を決定します。すべての値が整数なら INTEGER、浮動小数点数があれば REAL、文字列があれば TEXT、true/falseのみなら BOOLEAN として推論します。型が混在する場合は最も広い型（TEXT）が選ばれます。null値を含む行があるカラムはNULL許容（NOT NULL なし）になります。' },
      { title: '方言ごとの違い', body: 'PostgreSQLとSQLiteはダブルクォートでカラム名・テーブル名を囲みますが、MySQLはバッククォートを使います。BOOLEAN型はPostgreSQL/SQLiteではTRUE/FALSEリテラルを使いますが、MySQLはTINYINT(1)で0/1で格納します。浮動小数点数はMySQLではDOUBLEが一般的なので自動的に変換されます。' },
      { title: 'スキーマ名の活用', body: 'PostgreSQLでは`public.users`のようにスキーマ名を指定してテーブルを整理します。スキーマ名フィールドを入力すると`"schema"."table"`形式でCREATE TABLEとINSERT文が生成されます。MySQLの場合はデータベース名を指定することで`database`.`table`形式になります。' },
      { title: '大量データの投入', body: '生成されたINSERT文は全行を1つのINSERT INTO ... VALUES(...),(...),(...)形式（マルチロウINSERT）ではなく、行ごとに分けています。これにより1行ずつ実行して部分的にデバッグしやすくなります。大量データの本番投入にはPSQLの\COPYやMySQLのLOAD DATA INFILEがより高速です。' },
    ],
  },
  ...
  'totp-generator': {
    ...
    sections: [
      ...
      { title: '前後のコードが有効な理由', body: '時刻のずれ（クロックスキュー）に対応するため、多くの2FA実装では現在のコードに加えて前後1ステップ（±30秒）のコードも有効と見なします。このツールでは前・現在・次の3コードを表示するため、時刻がわずかにずれている環境でのデバッグに役立ちます。' },
      { title: 'OTP実装のデバッグ用途', body: 'アプリにTOTP認証を組み込む際、生成したコードが正しいか確認するためにこのツールが使えます。同じBase32シークレットを使って、実装したコードとこのツールの出力が一致していれば正しく実装されています。WebCrypto APIのHMAC-SHA1をブラウザで直接実行するため、通信なしで安全に確認できます。' },
    ],
  },
  ...
  'html-to-markdown': {
    ...
    sections: [
      ...
      { title: 'GFMテーブルの変換', body: 'HTML の `<table>` タグは GFM のパイプ記法テーブルに変換されます。`| ヘッダー1 | ヘッダー2 |` の形式でヘッダー行、`| --- | --- |` でセパレータ行、その後に各データ行が続きます。ヘッダーセルが `<th>` でなく `<td>` の場合でも最初の行はヘッダーとして扱います。' },
      ...
      { title: 'ネストしたリストの扱い', body: '`<ul>` と `<ol>` のネストは、インデント（2スペース）で表現されます。` - 親項目` の下に `  - 子項目` が続く形式です。`<li>` の中にさらに `<ul>` や `<ol>` がある場合も正しく再帰的に変換されます。' },
    ],
  },
