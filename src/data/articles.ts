...
export const articles: Article[] = [
  {
    ...
    sections: [
      {
        heading: '1. ベースカラーから配色（パレット）を作る',
        body: 'まずはベースとなる1色から配色全体を組み立てます。カラーパレットジェネレーターにベースカラーを入れると、補色・類似色・トライアドなど複数のカラーハーモニーを自動生成してくれます。感覚で色を並べるより、調和のとれた配色の候補を素早く得られます。ここで主要色（プライマリ・アクセント・背景・文字）のあたりを付けます。',
        tool: 'color-palette-generator',
      },
      {
        heading: '2. 文字と背景のコントラスト比を確認する',
        body: '配色候補が決まったら、文字色と背景色の組み合わせがちゃんと読めるかをコントラスト比で確認します。WCAG 2.1 では通常サイズの本文で 4.5:1 以上、大きな文字で 3:1 以上が推奨されています。コントラストチェッカーに2色を入れると比率と AA / AAA の判定が出るので、基準を満たさない組み合わせは明度を調整します。特にアクセントカラーの上に白文字を乗せるUIはコントラストが不足しがちなので要注意です。',
        tool: 'color-contrast-checker',
      },
      {
        heading: '3. 色覚特性でも区別できるか確認する',
        body: 'コントラストを満たしても、「赤と緑で状態を区別する」ような配色は色覚特性のある人には見分けにくいことがあります。色覚シミュレーターで配色を各タイプ（P型・D型など）で確認し、色だけに依存していないかをチェックします。区別がつきにくい場合は、色に加えてアイコン・ラベル・パターンなど色以外の手がかりを添えると、誰にとっても分かりやすくなります。',
        tool: 'color-blindness-simulator',
      },
      {
        heading: '4. 決めた配色をトークンとして出力する',
        body: '最終的な配色が固まったら、実装で使い回せる形に落とし込みます。カラートークンジェネレーターにパレットを入れると、CSS変数・Tailwind・Sass・Style Dictionary などの形式で一括出力できます。色をトークンとして一元管理しておくと、後からブランドカラーを変えるときも1箇所の修正で済み、デザインと実装の間で色がズレるのを防げます。',
        tool: 'color-token-generator',
      },
    ],
    conclusion:
