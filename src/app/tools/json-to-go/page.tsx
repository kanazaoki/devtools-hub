...
const tool = tools.find((t) => t.slug === 'json-to-go')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription('json-to-go', tool.description),
  alternates: {
    canonical: 'https://devtools-hub.dev/tools/json-to-go',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: getMetaDescription('json-to-go', tool.description),
    images: [{ url: '/tools/json-to-go/opengraph-image', width: 1200, height: 630 }],
  },
}
...
export default function JsonToGoPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="JSON to Go ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">JSON to Go</p>
        </div>
