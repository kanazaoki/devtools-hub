...
const tool = tools.find((t) => t.slug === 'json-path-tester')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription('json-path-tester', tool.description),
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/json-path-tester',
  },
  openGraph: {
...
export default function JsonPathTesterPage() {
  ...
  return (
    <main className="py-10">
      ...
      <AdSense slot="1010101026" format="horizontal" className="mb-8" />

      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="JSON Path Tester ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">JSON Path Tester</p>
        </div>
      ...
      <SeoArticle slug="json-path-tester" />

      <AdSense slot="1010101027" format="rectangle" className="mb-12" />
