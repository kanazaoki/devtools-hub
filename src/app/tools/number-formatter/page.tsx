...
const tool = tools.find((t) => t.slug === 'number-formatter')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription(tool.slug, tool.description),
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/number-formatter',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: getMetaDescription(tool.slug, tool.description),
    images: [{ url: '/tools/number-formatter/opengraph-image', width: 1200, height: 630 }],
  },
}
...
export default function NumberFormatterPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="Number Formatter ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">Number Formatter</p>
        </div>
      ...
      <SeoArticle slug="number-formatter" />
