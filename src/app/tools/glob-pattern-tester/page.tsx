...
import { GlobPatternTester } from '@/components/tools/GlobPatternTester'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { SeoArticle } from '@/components/SeoArticle'

const tool = tools.find((t) => t.slug === 'glob-pattern-tester')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: tool.description,
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/glob-pattern-tester',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: tool.description,
    images: [{ url: '/tools/glob-pattern-tester/opengraph-image', width: 1200, height: 630 }],
  },
}

export default function GlobPatternTesterPage() {
  ...
  return (
    <main className="py-10">
      ...
      <AdSense slot="8701234503" format="horizontal" className="mb-8" />

      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="Glob Pattern Tester ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">Glob Pattern Tester</p>
        </div>
      ...
      <SeoArticle slug="glob-pattern-tester" />

      <AdSense slot="8701234504" format="rectangle" className="mb-12" />
