...
import { CssLayerVisualizer } from '@/components/tools/CssLayerVisualizer'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { ToolBreadcrumb } from '@/components/ToolBreadcrumb'
import { SeoArticle } from '@/components/SeoArticle'
import { getMetaDescription } from '@/data/seo-content'

const tool = tools.find((t) => t.slug === 'css-layer-visualizer')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription('css-layer-visualizer', tool.description),
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/css-layer-visualizer',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: getMetaDescription('css-layer-visualizer', tool.description),
    images: [{ url: '/tools/css-layer-visualizer/opengraph-image', width: 1200, height: 630 }],
  },
}

export default function CssLayerVisualizerPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="CSS @layer Visualizer ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">CSS @layer Visualizer</p>
        </div>
      ...
      <SeoArticle slug="css-layer-visualizer" />
