...
import { HashStudio } from '@/components/tools/HashStudio'
...
const tool = tools.find((t) => t.slug === 'hash-studio')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription('hash-studio', tool.description),
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/hash-studio',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: getMetaDescription('hash-studio', tool.description),
    images: [{ url: '/tools/hash-studio/opengraph-image', width: 1200, height: 630 }],
  },
}
...
export default function HashStudioPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="Hash Studio ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">Hash Studio</p>
        </div>
