...
import { TextDeck } from '@/components/tools/TextDeck'
...
const tool = tools.find((t) => t.slug === 'text-deck')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription('text-deck', tool.description),
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/text-deck',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: getMetaDescription('text-deck', tool.description),
    images: [{ url: '/tools/text-deck/opengraph-image', width: 1200, height: 630 }],
  },
}

export default function TextDeckPage() {
    ...
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="text-deck ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">text-deck</p>
        </div>
