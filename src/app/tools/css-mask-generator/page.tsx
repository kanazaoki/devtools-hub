...
import { CssMaskGenerator } from '@/components/tools/CssMaskGenerator'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { ToolBreadcrumb } from '@/components/ToolBreadcrumb'
import { SeoArticle } from '@/components/SeoArticle'

const tool = tools.find((t) => t.slug === 'css-mask-generator')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: getMetaDescription(tool.slug, tool.description),
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/css-mask-generator',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: getMetaDescription(tool.slug, tool.description),
    images: [{ url: '/tools/css-mask-generator/opengraph-image', width: 1200, height: 630 }],
  },
}
...
export default function CssMaskGeneratorPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="CSS Mask Generator ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">CSS Mask Generator</p>
        </div>
      ...
      <SeoArticle slug="css-mask-generator" />
