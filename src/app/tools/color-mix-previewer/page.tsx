...
import { ColorMixPreviewer } from '@/components/tools/ColorMixPreviewer'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { SeoArticle } from '@/components/SeoArticle'

const tool = tools.find((t) => t.slug === 'color-mix-previewer')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: tool.description,
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/color-mix-previewer',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: tool.description,
    images: [{ url: '/tools/color-mix-previewer/opengraph-image', width: 1200, height: 630 }],
  },
}

export default function ColorMixPreviewerPage() {
  ...
  return (
    <main className="py-10">
      ...
      <AdSense slot="8701234505" format="horizontal" className="mb-8" />

      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="CSS color-mix() Previewer ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">CSS color-mix() Previewer</p>
        </div>
      ...
      <SeoArticle slug="color-mix-previewer" />

      <AdSense slot="8701234506" format="rectangle" className="mb-12" />
