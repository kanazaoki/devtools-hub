...
import { SvgPathVisualizer } from '@/components/tools/SvgPathVisualizer'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { ToolBreadcrumb } from '@/components/ToolBreadcrumb'
import { SeoArticle } from '@/components/SeoArticle'
import { getMetaDescription } from '@/data/seo-content'

const tool = tools.find((t) => t.slug === 'svg-path-visualizer')!
...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description: getMetaDescription('svg-path-visualizer', tool.description),
    images: [{ url: '/tools/svg-path-visualizer/opengraph-image', width: 1200, height: 630 }],
  },
}
...
export default function SvgPathVisualizerPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="SVG Path Visualizer ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">SVG Path Visualizer</p>
        </div>
      ...
      <SeoArticle slug="svg-path-visualizer" />
