...
import { CssTransformExplorer } from '@/components/tools/CssTransformExplorer'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { SeoArticle } from '@/components/SeoArticle'

const tool = tools.find((t) => t.slug === 'css-transform-explorer')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: tool.description,
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/css-transform-explorer',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: tool.description,
    images: [{ url: '/tools/css-transform-explorer/opengraph-image', width: 1200, height: 630 }],
  },
}

export default function CssTransformExplorerPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="CSS Transform Explorer ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">CSS Transform Explorer</p>
        </div>
        <div className="p-5 sm:p-6">
          <CssTransformExplorer />
        </div>
      </section>
      ...
      <SeoArticle slug="css-transform-explorer" />
