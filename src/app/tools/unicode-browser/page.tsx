...
import { UnicodeBrowser } from '@/components/tools/UnicodeBrowser'
import { ToolJsonLd } from '@/components/ToolJsonLd'
import { ToolBreadcrumb } from '@/components/ToolBreadcrumb'
import { SeoArticle } from '@/components/SeoArticle'

const tool = tools.find((t) => t.slug === 'unicode-browser')!

export const metadata: Metadata = {
  title: `${tool.name} — ${tool.tagline}`,
  description: tool.description,
  alternates: {
    canonical: 'https://devtools-hub.vercel.app/tools/unicode-browser',
  },
  openGraph: {
    title: `${tool.name} | devtools-hub`,
    description: tool.description,
    images: [{ url: '/tools/unicode-browser/opengraph-image', width: 1200, height: 630 }],
  },
}
...
export default function UnicodeBrowserPage() {
  ...
  return (
    <main className="py-10">
      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-border bg-surface" aria-label="Unicode Browser ツール">
        <div className="border-b border-border px-5 py-3">
          <p className="font-mono text-xs uppercase tracking-widest text-muted">Unicode Browser</p>
        </div>
      ...
      <SeoArticle slug="unicode-browser" />
